...
MIN_IMAGE_SIZE_WITH_PAD     = 0x100 * 1024  # Minimum image size for sbl1 Nand based OTA feature
...
ELFINFO_MAG0 =      0x7f
ELFINFO_MAG1 = ord(b'E')
ELFINFO_MAG2 = ord(b'L')
ELFINFO_MAG3 = ord(b'F')
ELFINFO_CLASS_INDEX       = 4
ELFINFO_CLASS_32          = 1
ELFINFO_CLASS_64          = 2
ELFINFO_VERSION_INDEX     = 6
ELFINFO_VERSION_CURRENT   = 1
ELF_BLOCK_ALIGN = 0x001000
...
# Segment Type
MI_PBT_L4_SEGMENT =        0
MI_PBT_AMSS_SEGMENT =      1
MI_PBT_HASH_SEGMENT =      2
MI_PBT_BOOT_SEGMENT =      3
MI_PBT_L4BSP_SEGMENT =     4
MI_PBT_SWAPPED_SEGMENT =   5
MI_PBT_SWAP_POOL_SEGMENT = 6
MI_PBT_PHDR_SEGMENT =      7
...
MI_PBT_RO_SEGMENT =      1
MI_PBT_ZI_SEGMENT =      2
MI_PBT_NOTUSED_SEGMENT = 3
MI_PBT_SHARED_SEGMENT =  4
...
MI_PBT_ELF_AMSS_RELOCATABLE_IMAGE =                0x08000000
# ----------------------------------------------------------------------------
...
class Elf32_Phdr:
	...
	def __init__(self, data):
		...
		self.unpacked_data = unpacked_data
		self.p_type = unpacked_data[0x0]
		self.p_offset = unpacked_data[0x1]
		self.p_vaddr = unpacked_data[0x2]
		self.p_paddr = unpacked_data[0x3]
...
class Elf64_Ehdr:
	...
	def __init__(self, data):
		...
		self.unpacked_data = unpacked_data
		self.e_ident = unpacked_data[0x0]
		self.e_type = unpacked_data[0x1]
		self.e_machine = unpacked_data[0x2]
		self.e_version = unpacked_data[0x3]
		self.e_entry = unpacked_data[4]
		self.e_phoff = unpacked_data[5]
		self.e_shoff = unpacked_data[6]
		self.e_flags = unpacked_data[7]
...
class Elf64_Phdr:
	...
	def __init__(self, data):
		...
		self.unpacked_data = unpacked_data
		self.p_type = unpacked_data[0x0]
		self.p_flags = unpacked_data[0x1]
		self.p_offset = unpacked_data[0x2]
		self.p_vaddr = unpacked_data[0x3]
...
def generate(env):
	...
	if 'USES_ENCRYPT_MBN' in env:
		...
		env.Tool('pil_encrypt',
				 toolpath=['${BUILD_ROOT}/core/securemsm/ssd/tools/pil_encrypt'
						   ])
...
def encrypt_mbn(env, mbn_in_file_name, mbn_out_file_name):
	...
	encrypted_buf = env.SSDEncryptSegment(0x0, file_buff, mbn_file_size)
...
# ----------------------------------------------------------------------------
def encrypt_elf_segments(env, elf_in_file_name, elf_out_file_name):
	...
	encrypted_seg_counter = 0x0

	# Copy input file to output file
	shutil.copyfileobj(elf_in_fp, elf_out_fp,
					   os.path.getsize(elf_in_file_name))
	...
	for i in range(elf_header.e_phnum):
		...
		if curr_phdr.p_type == LOAD_TYPE \
				and MI_PBT_SEGMENT_TYPE_VALUE(curr_phdr.p_flags) \
				!= MI_PBT_HASH_SEGMENT:
			...
			encrypted_buf = \
				env.SSDEncryptSegment(encrypted_seg_counter, file_buff,
									  data_len)
			encrypted_seg_counter += 0x1
...
def image_header(
	...
	if in_code_size is None and os.path.exists(code_file_name) is False:
		raise RuntimeError(
			'Code size unavailable, and input file does not exist')
	...
	if gen_dict['IMAGE_KEY_MBN_TYPE'] == 'elf':
		image_dest = get_hash_address(elf_file_name) \
			+ MI_BOOT_IMG_HDR_SIZE
	...
	if header_format == 'sbl':
		...
		boot_sbl_header.cert_chain_ptr = image_dest + code_size \
			+ signature_size
		boot_sbl_header.cert_chain_size = cert_chain_size
		boot_sbl_header.oem_root_cert_sel = \
			gen_dict['IMAGE_KEY_OEM_ROOT_CERT_SEL']
		boot_sbl_header.oem_num_root_certs = \
			gen_dict['IMAGE_KEY_OEM_NUM_ROOT_CERTS']
		...
		if requires_preamble is True:
			boot_sbl_header = image_preamble(gen_dict,
											 preamble_file_name, boot_sbl_header, num_of_pages)
		...
		boot_header.cert_chain_ptr = image_dest + code_size \
			+ signature_size
		boot_header.cert_chain_size = cert_chain_size

		# Package up the header and write to output file
		boot_header.writePackedData(target=output_file_name,
									write_full_hdr=write_full_hdr)
	elif header_format == 'reg' and mbn_version == 6:

		boot_header = Boot_Hdr_V6(init_val=int('0x0', 16))
		boot_header.image_id = gen_dict['IMAGE_KEY_IMAGE_ID']
		boot_header.qti_sig_size = 0x0
		boot_header.qti_cert_chain_size = 0x0
		boot_header.image_size = image_size
		boot_header.code_size = code_size
		boot_header.sig_ptr = 0x0
		boot_header.sig_size = signature_size
		boot_header.cert_chain_ptr = 0x0
		boot_header.cert_chain_size = cert_chain_size
		boot_header.oem_metadata_size = 0x0
		boot_header.qti_metadata_size = 0x0
		...
		raise RuntimeError(
			'Header format not supported: ' + str(header_format))
	return 0x0
...
def pboot_gen_elf(
	...
	if elf_out_file_name is not None:
		...
		if num_phdrs > MAX_PHDR_COUNT:
			raise RuntimeError(
				'Input ELF has exceeded maximum number of program headers')
	...
	for i in range(num_phdrs):
		...
		if MI_PBT_PAGE_MODE_VALUE(curr_phdr.p_flags) \
			...
			if seg_size & ELF_BLOCK_ALIGN - 0x1 > 0x0:
				raise RuntimeError(
					'seg_size: ' + hex(seg_size) + ' is not ELF page aligned!')
		...
		elif MI_PBT_PAGE_MODE_VALUE(curr_phdr.p_flags) \
			== MI_PBT_NON_PAGED_SEGMENT and curr_phdr.p_type \
				is not PHDR_TYPE:
			...
			data_len = curr_phdr.p_filesz
			file_buff = elf_in_fp.read(data_len)
			if is_com == True and (curr_phdr.p_type is LOAD_TYPE
								   or curr_phdr.p_type
								   == PHDR_ENTR_TYPE):
				com_data_len = 0x0
				if data_len > 0x0:
					...
					if prc.returncode != 0x0:
						print(
							'ERROR: unable to create compressed file Segment for' + str(Comfile))

					else:
						com_out_fp = OPEN(Comfile, 'rb')
						com_out_fp.seek(0x0, 0x2)
						com_data_len = com_out_fp.tell()
						iarrPhdrFileLen.append(com_data_len)
						com_out_fp.close()

					src_in_fp = OPEN(srcfile, 'rb')
					src_in_fp.seek(0x0, 0x2)
					data_len = src_in_fp.tell()
					src_in_fp.seek(0x0, 0x0)
	...
	if elf_out_file_name is not None:
		...
		if mbn_version == 6:
			[hash_Phdr, pad_hash_segment, hash_tbl_end_addr,
			 hash_tbl_offset] = initialize_hash_phdr(elf_in_file_name,
													 hashtable_size, MI_BOOT_IMG_HDR_SIZE_V6,
													 ELF_BLOCK_ALIGN, is_elf64)
		else:
			[hash_Phdr, pad_hash_segment, hash_tbl_end_addr,
			 hash_tbl_offset] = initialize_hash_phdr(elf_in_file_name,
													 hashtable_size, MI_BOOT_IMG_HDR_SIZE,
													 ELF_BLOCK_ALIGN, is_elf64)
		...
		min_offset = phdr_table[0x0].p_offset
		for i in range(num_phdrs):
			curr_phdr = phdr_table[i]
		...
		for i in range(num_phdrs):
			...
			if is_com == True:
				...
				if i > 0x0:
					Prev_phdr = phdr_table[i - 0x1]
					pAlign = curr_phdr.p_align
					Com_Filsize = iarrPhdrFileLen[i - 0x1]
				else:
					pAlign = curr_phdr.p_align
					Com_Filsize = 0x0
				...
				NextSeg_Modoffset = NextSeg_offset & pAlign - 0x1

				if NextSeg_Modoffset > 0x0:
					NextSeg_offset = NextSeg_offset + pAlign \
						- NextSeg_Modoffset
				...
				if (curr_phdr.p_type is LOAD_TYPE or curr_phdr.p_type
						== PHDR_ENTR_TYPE) and curr_phdr.p_filesz > 0x0:
					com_out_fp = OPEN(Comfile, 'rb')
					file_copy_offset(com_out_fp, 0x0, elf_out_fp,
									 iarrPhdrFileOff[i], iarrPhdrFileLen[i])
					com_out_fp.close()
				elif curr_phdr.p_filesz > 0x0:
					file_copy_offset(
						elf_in_fp, src_offset, elf_out_fp, iarrPhdrFileOff[i], iarrPhdrFileLen[i])
			if is_com == False and curr_phdr.p_filesz > 0x0:
				file_copy_offset(elf_in_fp, src_offset, elf_out_fp,
								 curr_phdr.p_offset + hashtable_shift, curr_phdr.p_filesz)
...
def pboot_add_hash(
	...
	if hash_segment_found is True:
		...
		file_copy_offset(hash_tbl_fp, 0x0, elf_out_fp,
						 hash_hdr_offset, hash_size)
	else:
		raise RuntimeError(
			'Hash segment program header not found in file ' + elf_in_file_name)
...
def image_auth(env, *args):
	...
	binary_in = args[0x0]
	signature = args[0x1]
	attestation_cert = args[0x2]
	attestation_ca_cert = args[0x3]
	root_cert = args[4]
	...
	concat_files(signed_image_out, [binary_in, signature,
									cert_chain_out])
...
def modify_relocatable_flags(env, output_elf):
	...
	new_align = curr_phdr.p_align & 0x0 | alignment_value
	...
	for i in range(elf_header.e_phnum):
		...
		curr_phdr = phdr_table[i]
		new_flag = curr_phdr.p_flags & ~MI_PBT_FLAGS_MASK \
...
# modify_elf_flags
# ----------------------------------------------------------------------------
def modify_elf_flags(env, elf_in_file_name, scl_file_name):
	...
	if len(segment_list) is not elf_header.e_phnum:
		raise RuntimeError(
			'SCL file and ELF file have different number of segments!')
...
def generate_code_hash(env, elf_in_file_name):
	...
	for i in range(elf_header.e_phnum):
		...
		if curr_phdr.p_align == 8 and curr_phdr.p_flags & PH_PERM_MASK \
			== PH_PERM_RW and curr_phdr.p_filesz != 0x0 \
				and curr_phdr.p_filesz % DP_HASH_SIZE == 0x0:
			hash_seg_idx = i
			...
			while len(hash_data) < curr_phdr.p_filesz:
				hash_data = hash_data \
					+ elf_in_fp.read(curr_phdr.p_filesz - len(hash_data))

			hash_data = struct.unpack('I' * (curr_phdr.p_filesz / 4),
									  hash_data)
	...
	for i in range(elf_header.e_phnum):
		curr_phdr = phdr_table[i]
		curr_pages = (curr_phdr.p_filesz + DP_PAGE_SIZE - 0x1) \
			/ DP_PAGE_SIZE

		if curr_phdr.p_align == DP_CODE_ALIGN and curr_phdr.p_flags \
			& PH_PERM_MASK == PH_PERM_RX and curr_pages \
				== code_seg_pages:
			if code_seg_idx != -0x1:
				raise RuntimeError(
					'Multiple code segments match for: ' + code_seg_pages + ' pages')
			code_seg_idx = i
	...
	while bytes_left > 0x0:
		...
		if len(page) < DP_PAGE_SIZE:
			page = page + struct.pack('b', 0x0) * (DP_PAGE_SIZE - len(page))
			...
			if bracket_counter == 0x0:
				...
				try:
					...
					if start_addr in global_dict:
						...
						raise RuntimeError(
							'Unrecognized start address: ' + start_addr)
...
def getSegmentFlag(seg_info):
	...
	if NOTPAGEABLE in seg_info:
		...
		raise RuntimeError(
			'The segment name is wrongly defined in the SCL file: ' + seg_info)
...
def init_build_vars(env):
	...
	CERT_CHAIN_ONEROOT_MAXSIZE = get_dict_value(env['GLOBAL_DICT'],
												'CERT_CHAIN_MAXSIZE', 6 * 1024)

	# Maximum size of the XML Header used in encrypted ELF images
	global XML_HEADER_MAXSIZE
	XML_HEADER_MAXSIZE = get_dict_value(env['GLOBAL_DICT'],
										'XML_HEADER_MAXSIZE', 0x2 * 1024)
...
def generate_global_dict(env):
	...
	if os.path.exists(cust_file_name) is True \
			and os.path.exists(targ_file_name) is True:

		# Populate the dictionary from the auto-generated files
		global_dict = populate_dictionary(targ_file_name,
										  cust_file_name)
	else:
		global_dict = {}
...
def populate_dictionary(*args):
	...
	for i in range(len(args)):
		...
		for values in itertools.izip(values):
			new_entry = values[0x0]

			# Verify the parsed tokens
			if new_entry['Define'] == '#define' and new_entry['Key'] \
					!= None and new_entry['Value'] != None:
				...
				if global_dict is not None and len(global_dict.keys()) \
						> 0x0:
					for key in global_dict:
						new_value = new_value.replace(key,
													  str(global_dict.get(key)))
...
def filter_dictionary(env, global_dict, **kwargs):
	...
	if not kwargs.has_key('IMAGE_TYPE'):
		raise RuntimeError(
			'IMAGE_TYPE must be defined to use FilterDictionary.')
	else:
	...
	if image_type not in image_id_table:
		...
		id = image_id_table[image_type][0x0]
		id_match_str = image_id_table[image_type][0x1]
	...
	if id_mbn_type == 'elf':
		...
		if template_key_match in global_dict:
			...
			raise RuntimeError(
				'Builds file does not have IMAGE_KEY pair for: ' + image_type)
	else:
		raise RuntimeError('MBN_TYPE currently not supported: ' + mbn_type)
	...
	gen_dict['IMAGE_KEY_BOOT_SMALL_PREAMBLE'] = \
		get_dict_value(global_dict, 'BOOT_SMALL_PREAMBLE', 0x1)

	# Get OEM root certificate select and number
	oem_root_cert_sel = get_dict_value(global_dict, 'OEM_ROOT_CERT_SEL', 0x1)
	oem_num_root_certs = get_dict_value(global_dict,
										'OEM_NUM_ROOT_CERTS', 0x1)

	# Error checking for OEM configurable values
	if oem_root_cert_sel in range(0x1, MAX_NUM_ROOT_CERTS + 0x1) \
			and oem_num_root_certs in range(0x1, MAX_NUM_ROOT_CERTS + 0x1) \
			and oem_root_cert_sel <= oem_num_root_certs:
...
def get_hash_address(elf_file_name):
	...
	for i in range(elf_header.e_phnum):
		...
		if curr_phdr.p_paddr > last_paddr:
			...
			if MI_PBT_SEGMENT_TYPE_VALUE(curr_phdr.p_flags) \
					!= MI_PBT_SWAPPED_SEGMENT:
				last_paddr = curr_phdr.p_paddr
				last_paddr_segment = i

	max_phdr = phdr_table[last_paddr_segment]

	ret_val = (max_phdr.p_paddr + max_phdr.p_memsz - 0x1
			   & ~(ELF_BLOCK_ALIGN - 0x1)) + ELF_BLOCK_ALIGN
...
def verify_elf_header(elf_header):
	...
	if elf_header.e_ident[ELFINFO_MAG0_INDEX] != ELFINFO_MAG0:
		ret = 1
	elif elf_header.e_ident[ELFINFO_MAG1_INDEX] != ELFINFO_MAG1:
		ret = 2
	elif elf_header.e_ident[ELFINFO_MAG2_INDEX] != ELFINFO_MAG2:
		ret = 3
	elif elf_header.e_ident[ELFINFO_MAG3_INDEX] != ELFINFO_MAG3:
		ret = 4
	elif elf_header.e_ident[ELFINFO_CLASS_INDEX] != ELFINFO_CLASS_64 \
			and elf_header.e_ident[ELFINFO_CLASS_INDEX] != ELFINFO_CLASS_32:
		ret = 5
	elif elf_header.e_ident[ELFINFO_VERSION_INDEX] != ELFINFO_VERSION_CURRENT:
		ret = 6

	if ret != 0:
		raise RuntimeError("verify elf header failed with error: %d\n" % ret)
		return False
	else:
		return True
...
def initialize_hash_phdr(
	...
	pad_hash_segment = hash_tbl_end_addr & ELF_BLOCK_ALIGN - 0x1
	...
	hash_Phdr.p_memsz = hash_hdr_size + hash_tbl_size + ELF_BLOCK_ALIGN \
		- pad_hash_segment
	...
	return [hash_Phdr, pad_hash_segment, hash_tbl_end_addr,
			hash_tbl_offset]
...
def image_preamble(
	...
	autodetectpage[0x0] = FLASH_CODE_WORD
	autodetectpage[0x1] = MAGIC_NUM
	if num_of_pages == 64:
		autodetectpage[0x2] = AUTODETECT_PAGE_SIZE_MAGIC_NUM64
	elif num_of_pages == 128:
		autodetectpage[0x2] = AUTODETECT_PAGE_SIZE_MAGIC_NUM128
	else:
		autodetectpage[0x2] = AUTODETECT_PAGE_SIZE_MAGIC_NUM
	...
	if gen_dict['IMAGE_KEY_BOOT_SMALL_PREAMBLE'] == 0x1:
		boot_sbl_header.image_src += flash_max_page + flash_min_page
		amount_to_write = flash_min_page
	else:
		boot_sbl_header.image_src += flash_max_page * 0x2
...
def MI_PBT_SEGMENT_TYPE_VALUE(x):
	return (x & MI_PBT_FLAG_SEGMENT_TYPE_MASK) \
		>> MI_PBT_FLAG_SEGMENT_TYPE_SHIFT


def MI_PBT_PAGE_MODE_VALUE(x):
	return (x & MI_PBT_FLAG_PAGE_MODE_MASK) \
		>> MI_PBT_FLAG_PAGE_MODE_SHIFT


def MI_PBT_ACCESS_TYPE_VALUE(x):
	return (x & MI_PBT_FLAG_ACCESS_TYPE_MASK) \
		>> MI_PBT_FLAG_ACCESS_TYPE_SHIFT


def MI_PBT_CHECK_FLAG_TYPE(x):
	return MI_PBT_SEGMENT_TYPE_VALUE(x) != MI_PBT_HASH_SEGMENT \
		and MI_PBT_ACCESS_TYPE_VALUE(x) != MI_PBT_NOTUSED_SEGMENT \
		and MI_PBT_ACCESS_TYPE_VALUE(x) != MI_PBT_SHARED_SEGMENT
...
def insert_SBL1_magicCookie(env, target):
	...
	if length <= VIRTUAL_BLOCK_SIZE:
		...
		while length > VIRTUAL_BLOCK_SIZE:
			filedata_till_128kb = filedata[0x0:VIRTUAL_BLOCK_SIZE]
			filedata_after_128kb = filedata[VIRTUAL_BLOCK_SIZE:length]

			a = str(hex(FLASH_CODE_WORD))
			mc1 = chr(int(a[8:10], 16)) + chr(int(a[6:8], 16)) \
				+ chr(int(a[4:6], 16)) + chr(int(a[0x2:4], 16))

			b = str(hex(MAGIC_NUM))
			mc2 = chr(int(b[8:10], 16)) + chr(int(b[6:8], 16)) \
				+ chr(int(b[4:6], 16)) + chr(int(b[0x2:4], 16))

			c = str(hex(SBL_VIRTUAL_BLOCK_MAGIC_NUM))
			mc3 = chr(int(c[8:10], 16)) + chr(int(c[6:8], 16)) \
				+ chr(int(c[4:6], 16)) + chr(int(c[0x2:4], 16))
...
def remove_SBL1_magicCookie(env, target, dest):
	...
	while length > VIRTUAL_BLOCK_SIZE:
		...
		filedata_after_128kb = filedata[VIRTUAL_BLOCK_SIZE
										+ MAGIC_COOKIE_LENGTH:length]
...
def pad_SBL1_image(
	env,
	target,
	min_size_with_pad=MIN_IMAGE_SIZE_WITH_PAD,
	page_size=0x0,
	num_of_pages=0x0,
):
	file = open(target, 'rb')
	...
	if length > alignment and alignment > 0x0:
		import math
